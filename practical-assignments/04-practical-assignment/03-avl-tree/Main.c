...
Node *newNode(Pokemon *pokemon) { return setNode(pokemon, NULL, NULL, 1); }

int getLevel(Node *node) { return (node == NULL) ? 0 : node->level; }

void setLevel(Node *node) {
    if (node != NULL) {
        node->level = 1 + (getLevel(node->left) > getLevel(node->right)
                               ? getLevel(node->left)
                               : getLevel(node->right));
    }
}
...
Node *balance(Node *node) {
    if (node != NULL) {
        ...
        if (factor >= -1 && factor <= 1) {
            ...
            int sonRightFactor =
                getLevel(node->right->right) - getLevel(node->right->left);
            ...
            int sonLeftFactor =
                getLevel(node->left->right) - getLevel(node->left->left);
            ...
            printf("Erro no No(%s) com fator de balanceamento (%d) inválido.",
                   node->pokemon->name, factor);
            exit(0);
        }
    }
...
void insert(Pokemon *pokemon) { root = recursiveInsert(pokemon, root); }
