...
/**
 * A função realiza uma ordenação parcial do vetor de Pokemon usando o algoritmo 
 * de inserção. Ela organiza os Pokemons com base na data de captura e, em caso de 
 * datas iguais, pelo nome, até o índice k.
 * 
 * @param findPokemon Um array de Pokemons a ser ordenado.
 * @param size O tamanho total do array de Pokemons.
 * @param k O índice até onde a ordenação deve ser aplicada.
 * @param comparisons Um ponteiro para uma variável que conta o número de comparações realizadas.
 * @param movements Um ponteiro para uma variável que conta o número de movimentações realizadas.
 */
void partialInsertionSort(Pokemon findPokemon[], int size, int k, int *comparisons, int *movements) {
...
/**
 * O ponto de entrada do programa que realiza a leitura dos dados dos Pokemons 
 * de um arquivo CSV, busca por Pokemons com IDs fornecidos pelo usuário, 
 * ordena os primeiros k Pokemons encontrados usando uma ordenação parcial 
 * por inserção e exibe suas informações. Ao final, registra o número de 
 * comparações e movimentações realizadas, juntamente com o tempo de execução, 
