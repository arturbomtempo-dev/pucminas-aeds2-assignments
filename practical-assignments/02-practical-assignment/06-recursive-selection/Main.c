...
/**
 * Ordena recursivamente um array de ponteiros para Pokémons usando o algoritmo de seleção.
 *
 * @param pokemon Array de ponteiros para Pokémons.
 * @param n Número de Pokémons no array.
 * @param comparisons Ponteiro para o contador de comparações realizadas durante a ordenação.
 * @param movements Ponteiro para o contador de movimentos de troca realizados durante a ordenação.
 * @param start Índice inicial para o início da seleção recursiva.
 */
void recursiveSelectionSort(Pokemon **pokemon, int n, int *comparisons, int *movements, int start) {
    if (start <= n - 1) {
        int smaller = start;
...
/**
 * Função principal que executa a leitura, ordenação e exibição dos dados de Pokémon.
 */
int main() {
    clock_t start, end;
    double totalTime;
    ...
    saveExecutionFile("847235_selecaoRecursiva.txt", comparisons, movements, totalTime);
